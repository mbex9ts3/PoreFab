...
% $Revision: 1.4 $  $Date: 2013/08/06 14:31:16 $
  ...
  %try
      ...
      dataIn = paramsStruct.data
      
      setappdata(findobj('Tag', 'sliceView'), 'imLogi', false(size(dataIn,1),1)); 
      setappdata(findobj('Tag', 'sliceView'), 'dataIn', dataIn); 
      ...
      try
          ...
          set(handle(getOriginalModel(jtable),'CallbackProperties'), 'TableChangedCallback', {@tableChangedCallback, jtable}); %enabled
...
trueRow = modifiedRow+1;
trueCol = modifiedCol+1;

% load checkbox history
imLogi = getappdata(findobj('Tag', 'sliceView'), 'imLogi');
dataIn = getappdata(findobj('Tag', 'sliceView'), 'dataIn');

% check event is checkbox
if trueCol == 3 % checkbox hit
    
    % find index of previous checkbox
    idx  = find(imLogi == true);
    if isempty(idx) % no previous image displayed
        disp('condition 1');
        imLogi(trueRow) = true;
        imStore = getappdata(findobj('Tag', 'sliceView'), 'imStore');
%         imStoreNP = getappdata(findobj('Tag', 'sliceView'), 'imStoreNP');
%         maskStore = getappdata(findobj('Tag', 'sliceView'), 'maskStore');
        im = imStore{imLogi};
%         imNP = imStoreNP{imLogi};
%         mask = maskStore{imLogi};
        setappdata(findobj('Tag', 'sliceView'), 'imCurrent', im); 
%         setappdata(findobj('Tag', 'sliceView'), 'imCurrentNP', imNP); 
        setappdata(findobj('Tag', 'sliceView'), 'imLogi', imLogi); 
%         setappdata(findobj('Tag', 'sliceView'), 'maskCurrent', mask);
        
    elseif idx  == trueRow && newData == false % reset current checkbox to off
        disp('condition 2');
        imLogi(trueRow) = false;
        setappdata(findobj('Tag', 'sliceView'), 'imCurrent', nan);
%         setappdata(findobj('Tag', 'sliceView'), 'imCurrentNP', nan);
        setappdata(findobj('Tag', 'sliceView'), 'imLogi', imLogi);
%         setappdata(findobj('Tag', 'sliceView'), 'maskCurrent', nan);
        
    elseif idx  ~= trueRow && newData == true % new selection
        disp('condition 3');
      
        % reset previous checkbox
        jtable.setValueAt(false, idx, 1)
        
        % collect image data
        imLogi = false(size(imLogi,1),1);
        imLogi(trueRow) = true; 
        imStore = getappdata(findobj('Tag', 'sliceView'), 'imStore');
%         imStoreNP = getappdata(findobj('Tag', 'sliceView'), 'imStoreNP');
%         maskStore = getappdata(findobj('Tag', 'sliceView'), 'maskStore');
