...
% Last Modified by GUIDE v2.5 05-Sep-2019 11:34:55
...
togT = imread('togT2.png');
set(handles.btnFlicker,'CData',togT);
saveBin = imread('saveBin.png');
set(handles.btnOK,'CData',saveBin);
zoomMag = imread('zoomT.png');
set(handles.zoomT,'CData',zoomMag);
panHand = imread('panHandT.png');
set(handles.panT,'CData',panHand);
setappdata(gcf,'zoomFlag',false);   
...
global g_MinThreshold;
global g_LastThresholdedColorBand;
g_LastThresholdedColorBand = 0; %get(handles.listbox_DisplayColorBand,'Value') - 1;
try
...
try
	...
	axes(handles.axes2);
	colorBand = 1; %get(handles.listbox_DisplayColorBand,'Value');
...
global numberOfColorBands;
try
	radioButtonSetting = 1; % get(handles.radBlackOutside,'Value');
...
% 		set(handles.txtInfo,'string','Displaying images');
		colorBand = 0; %get(handles.listbox_DisplayColorBand,'Value');
...
try
	...
	if length(imageDimensions) == 3
		...
		ax = gca;
		ax.XColor = 'r';
		xlabel('P', 'Color', 'white');
		ax.YColor = 'r';
		ylabel('N', 'Color', 'white');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
zoomFlag = getappdata(gcf, 'zoomFlag');
if zoomFlag == false
%     all_ha = findobj(gcf, 'type', 'axes', 'tag', '' );
%     linkaxes( all_ha, 'xy');
    zoom on;
    setappdata(gcf, 'zoomFlag', true);
else
    zoom off;
    setappdata(gcf, 'zoomFlag', false);
end
...
% --- Executes on button press in panT.
function panT_Callback(hObject, eventdata, handles)
% hObject    handle to panT (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
